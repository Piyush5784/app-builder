...
4a. Never append "|| true", "; true", "|| exit 0", or redirect stderr away just to force a verification command to report success — that only hides the real exit code, it does not fix anything. A build is only "done" when it exits 0 on its own, with no such suffix.
