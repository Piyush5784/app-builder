...
1. Fresh E2B sandbox created; the _existing_ `agent_sessions` row's `sandboxId` is updated to point at it (same row, not a new one).
...
session's history) _and_ to the specific `llm_calls` row that requested it
